...
# Stats
# basic linear regression
stats6Hoerner <- lm(`drag coefficient C_D` ~ `Reynolds number`, data = normal_effort)
summary(stats6Hoerner)

# Generalized linear mixed model
GLMM6Hoerner_mean <- lmer(`drag coefficient C_D` ~ `Reynolds number` + (1|sp_abbr), data = normal_effort)
summary(GLMM6Hoerner_mean)
r.squaredGLMM(GLMM6Hoerner_mean)

# Stats
# basic linear regression
stats6Emp <- lm(mean_drag ~ mean_Re, data = normal_effort)
summary(stats6Emp)

# Generalized linear mixed model
GLMM6Emp_mean <- lmer(mean_drag ~ mean_Re + (1|Species), data = normal_effort)
summary(GLMM6Emp_mean)
r.squaredGLMM(GLMM6Emp_mean)
